<?php
// /localhost/Proyecto_Instituto_Gestion/ProyectoProfesores_01.php
...
<html>
    <head>
        <meta charset="UTF-8">
        <!-- title>Instituto Sant Francesc - Control de faltas de asistencia por UF</title -->
        <title>IFP - Control de faltas de asistencia por UF</title>
        <link rel="stylesheet" href="Proyecto_Instituto_CSS/Proyecto_Instituto_Estilos.css">
    </head>
    <body class="body_1">
       <FONT COLOR=purple size=4>
          <center>
          <img src="Proyecto_Instituto_Imagenes/IFP.jpg" /> <!-- Imagen. -->
          <!-- h5 id="font_size_1" class="color_azul h_5">IFP</h5> <!-- Asigno varias clases en un mismo elemento: Ambas dentro de los mismas comillas separadas por espacio -->
          <h5 id="font_size_1" class="color_purpura h_5">Control de faltas de asistencia por UF</h4> <!-- Asigno varias clases en un mismo elemento: Ambas dentro de los mismas comillas separadas por espacio -->
          <!-- h4 id="font_size_1">Instituto Sant Francesc - Control de faltas de asistencia por UF</h4 -->
          <p align=right><a href="Proyecto_Instituto.html"><FONT size=4 color=green>Inicio</FONT></a></p> <!-- Enlace link mediante texto en la misma pestaña , Primero el link y despues el texto -->
          <p id="font_size_2"><u>Mantenimiento de profesores</u></p>
          </center>
       </FONT>
       ...
       <form method="POST" action="<?=$_SERVER["PHP_SELF"]?>">
          <input type="hidden" name="accion" value="mostrar_todos"> <!-- hidden sirve para if(isset($_POST['name=accion']) && $_POST['name=accion'] == 'value=mostrar_todos'){ -->
          ...
          <input type="submit" value="Mostrar todos" class="color_azul" /><br> <!-- Boton -->
       </form>
...
?>
       ...
       <form method="POST" action="<?=$_SERVER["PHP_SELF"]?>">
          <br><input type="hidden" name="accion" value="alta_profesor"> <!-- hidden sirve para if(isset($_POST['name=accion']) && $_POST['name=accion'] == 'value=alta_profesor'){ -->
          <input type="hidden" name="msg" value="<?php echo htmlspecialchars(serialize($msg), ENT_QUOTES); ?>" /> <!-- OJO unserialize($_POST['clase'])  ;  Google: storing php objects on form -> https://stackoverflow.com/questions/4842140/storing-php-objects-on-html-form-element-and-passing-php-objects-through-get-met -->
          <input type="submit" value="Alta de profesor" class="color_verde" /><br><br> <!-- Boton -->
       </form>
...
?>
       ...
       <form method="POST" action="<?=$_SERVER["PHP_SELF"]?>">
       <!-- form name="buscar_profesor_form" id="buscar_profesor_form" method="post" action="" -->
          Código de profesor: <input type="text" name="Id_Profesor" size="5"> <!-- id es para el foreach posterior -->
          &nbsp; Nombre de profesor: <input type="text" name="Nombre"> <!-- -->
          &nbsp; Apellido: <input type="text" name="Apellido"> <!-- -->
          &nbsp; Experiencia: <input type="text" name="Experiencia"> &nbsp; <!-- -->

          <input type="hidden" name="accion" value="buscar_profesor"> <!-- hidden sirve para if(isset($_POST['name=accion']) && $_POST['name=accion'] == 'value=buscar_profesor'){  -->
          <input type="hidden" name="msg" value="<?php echo htmlspecialchars(serialize($msg), ENT_QUOTES); ?>" /> <!-- OJO unserialize($_POST['clase'])  ;  Google: storing php objects on form -> https://stackoverflow.com/questions/4842140/storing-php-objects-on-html-form-element-and-passing-php-objects-through-get-met -->
          <input type="submit" value="Buscar profesor" name="buscar_profesor" class="color_marron" /> <br><br> <!-- Boton -->
<?php
   if(isset($_POST['accion']) && $_POST['accion'] == 'modificar_profesor'){ // input type="hidden" name="accion"  ;  Si ha pulsado clicado boton MODIFICAR_PROFESOR
...
?>
       ...
       <form method="POST" action="<?=$_SERVER["PHP_SELF"]?>">
          Código de profesor: <input type="text" name="Id_Profesor"> &nbsp; <!-- id es para el foreach posterior -->
          <input type="hidden" name="accion" value="eliminar_profesor"> <!-- hidden sirve para if(isset($_POST['name=accion']) && $_POST['name=accion'] == 'value=eliminar_profesor'){ -->
          <input type="hidden" name="msg" value="<?php echo htmlspecialchars(serialize($msg), ENT_QUOTES); ?>" /> <!--  // OJO unserialize($_POST['clase'])  ;  Google: storing php objects on form -> https://stackoverflow.com/questions/4842140/storing-php-objects-on-html-form-element-and-passing-php-objects-through-get-met -->
          <input type="submit" value="Eliminar profesor" class="color_rojo" /><br><br> <!-- Boton -->

       <center><FONT size=3 color=blue><br><br><br><br><br>iFP 2019 ©
          <address><a href="mailto:ignaciomacipe@ifp.com?subject=Consulta informativa sobre un curso">ignaciomacipe@ifp.com</a></address> <!-- address mailto email italica -->
       </FONT></center>
       </form>
